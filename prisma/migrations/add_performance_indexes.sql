...
-- Note: MaintenanceRequest already has composite index (status, priority, propertyId) in schema
...
-- Index for property maintenance requests (covered by existing schema index)
-- CREATE INDEX IF NOT EXISTS "MaintenanceRequest_propertyId_status_idx" ON "MaintenanceRequest"("propertyId", "status");

-- ============== INQUIRY INDEXES ==============
-- Note: Inquiry already has composite index (status, priority, createdAt) in schema
...
-- ============== DOCUMENT INDEXES ==============
-- Note: DocumentFolder already has indexes defined in schema (userId, tenantId, propertyId, parentFolderId)
